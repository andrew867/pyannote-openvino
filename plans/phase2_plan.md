...
- Use representative inputs (segmentation: variable-length spectrogram frames; embedding: standardized chunk length) to produce ONNX files with dynamic axes documented.
...
- Run `python export_embedding.py` with representative embeddings input and verify load succeeds.
...
- `export_embedding.py`: loads embedding weights, exports ONNX with fixed or documented dynamic axis for batch/time.
